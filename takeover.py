...
TELEGRAM_TOKEN = config("TAKEOVER_TELEGRAM_TOKEN", default="CHANGEME")
TELEGRAM_CHAT_ID = config("TAKEOVER_TELEGRAM_CHAT_ID", default="CHANGEME")
NOTIFY_TELEGRAM = config("TAKEOVER_NOTIFY_TELEGRAM", default=True, cast=bool)
...
def banner():
    print("\n   /~\\")
    print("  C oo   ---------------")
    print(" _( ^)  |T|A|K|E|O|V|E|R| |A|L|E|R|T| ")
    print("/   ~\\  ----------------")
    print(" By @vikzsharma - Forked from https://github.com/m4ll0k/takeover  | Credits : M'hamed (@m4ll0k) Outaadi")
    print("-"*40)
        ...
        if output:
            ...
            if v and not k_.get('d_list'):
                plus('%s service found! Potential domain takeover found! - %s' %
                     (service, domain))
                notify_telegram(service, domain, error)
...
def notify_telegram(service, domain, error):
    log_entry = " Sub Domain Take over found at [ {} ] running [ {} ]".format(domain,service, error)
    payload = {
        'chat_id': TELEGRAM_CHAT_ID,
        'text': log_entry,
    }
    sendalert = requests.post("https://api.telegram.org/bot{token}/sendMessage".format(token=TELEGRAM_TOKEN),data=payload)

def notify(service, domain, error):  
    if NOTIFY_TELEGRAM:
        notify_telegram(service, domain, error)
